        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 max-w-xs">
          ...
          <div className="space-y-1">
            {payload.map((item, index) => {
              ...
              const label = isTraining ? 'Train' : 'Val';
              
              return (
                <p key={index} className="text-sm flex items-center justify-between gap-4">
                  <span className="flex items-center gap-2">
                    <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: models[modelKey].color }}></span>
    ...
    <div className="max-w-6xl mx-auto space-y-12">
      ...
      <div className="text-center">
        ...
        <p className="text-lg text-gray-600">
          We trained four different architectures to classify e-waste images. 
          By comparing their validation accuracy curves, you can see why {models.modelD.fullName} ({models.modelD.name}) 
          outperforms all other architectures with the highest validation accuracy and excellent generalization.
        </p>
      </div>
          ...
          <LineChart data={trainingData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            {showTraining && visibleModels.map((modelKey) => (
              <Line
                ...
                name={`${models[modelKey].name} (Train)`}
                stroke={models[modelKey].color}
                strokeWidth={1.5}
                strokeDasharray="5 5"
                dot={false}
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-emerald-800">
                  {models.modelD.name} (green line) achieves the highest validation accuracy of {(models.modelD.finalVal * 100).toFixed(0)}%, 
                  significantly outperforming {models.modelA.name} ({(models.modelA.finalVal * 100).toFixed(0)}%), {models.modelB.name} ({(models.modelB.finalVal * 100).toFixed(0)}%), 
                  and {models.modelC.name} ({(models.modelC.finalVal * 100).toFixed(0)}%). Notice its consistent performance throughout training.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
            ...
            {selectedModel === 'modelD' && (
              <div className="flex items-center gap-2 px-4 py-2 bg-emerald-100 text-emerald-800 rounded-lg flex-shrink-0">
            ...
            <div>
              ...
              <span className={`inline-block px-3 py-1 text-sm font-medium rounded-full ${
                selectedModel === 'modelD'
                  ...
                  : models[selectedModel].status.includes('High Performance')
                  ? 'bg-blue-100 text-blue-800'
                  : models[selectedModel].status.includes('Well Generalized')
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="space-y-3 text-sm">
            <p className="text-gray-600">
              <span className="font-semibold">{models.modelA.name}:</span> Shows consistent underfitting with validation accuracy plateauing around {(models.modelA.finalVal * 100).toFixed(0)}%. 
              Limited capacity to learn complex patterns.
            </p>
            <p className="text-gray-600">
              <span className="font-semibold">{models.modelB.name}:</span> Transfer learning approach achieves {(models.modelB.finalVal * 100).toFixed(0)}% validation accuracy 
              with well-aligned training and validation curves, indicating good generalization.
            </p>
            <p className="text-gray-600">
              <span className="font-semibold">{models.modelC.name}:</span> Transformer architecture reaches {(models.modelC.finalVal * 100).toFixed(0)}% validation accuracy, 
              demonstrating strong performance with global feature learning.
            </p>
            <p className="text-gray-600">
              <span className="font-semibold">{models.modelD.name}:</span> Detection-based approach achieves the best validation accuracy of {(models.modelD.finalVal * 100).toFixed(0)}% 
              with excellent balance between training and validation performance.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-3 text-sm">
            <div className="flex items-start gap-2">
              ...
              <p className="text-gray-700">
                <span className="font-semibold">Highest validation accuracy:</span> Achieves {(models.modelD.finalVal * 100).toFixed(1)}% validation accuracy, 
            ...
            <div className="flex items-start gap-2">
              ...
              <p className="text-gray-700">
                <span className="font-semibold">Excellent generalization:</span> Shows strong convergence from early epochs, 
                reaching high accuracy quickly and maintaining stability throughout training.
              </p>
            </div>
            <div className="flex items-start gap-2">
              <svg className="w-5 h-5 text-emerald-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-gray-700">
                <span className="font-semibold">Real-time capability:</span> YOLO architecture provides both high accuracy 
                and fast inference speed, ideal for production deployment.
              </p>
            </div>
          </div>
        </div>
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                      key === 'modelD'
                        ? 'bg-emerald-100 text-emerald-800'
                        : model.status.includes('High Performance')
                        ? 'bg-blue-100 text-blue-800'
                        : model.status.includes('Well Generalized')
                        ? 'bg-orange-100 text-orange-800'
                        : 'bg-yellow-100 text-yellow-800'
                    }`}>
                      {model.status}
